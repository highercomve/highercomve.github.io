(function (window) {
  ...
  `;
	...
	function updateAttributes(newNode, oldNode) {
		if (
			newNode.nodeType !== ELEMENT_NODE ||
			oldNode.nodeType !== ELEMENT_NODE
		) {
			return;
		}
	...
	function emptyTextNode(node) {
		return (
			node && node.nodeType === TEXT_NODE && node.nodeValue.trim() === ""
		);
	}
	...
	function updateElement($parent, newNode, oldNode) {
		...
		if (
			isTypeOfComponentBuilder(newNode) &&
			isTypeOfComponentBuilder(oldNode)
		) {
	...
	function setEvents(events) {
		...
		Object.keys(events || {}).forEach((key) => {
			...
			var handler = function (event) {
				var isChildrenOf = [...this.querySelectorAll(selector)].some(
					(element) => {
				...
				if (
					event.target &&
					(isChildrenOf || event.target.matches(selector))
				) {
					cb.call(this, event);
				}
			}.bind(this);
	...
	function updateComponent() {
		...
		var container = this;
		var newContent =
			this.renderTemplate.content.cloneNode(true).children[0];
	...
	window.CreateComponent = function (
		...
		Component.prototype.disconnectedCallback = disconnectedWrapper(
			options.disconnectedCallback ||
				consoleThis.bind(null, "disconnectedCallback"),
		);
		Component.prototype.attributeChangedCallback =
			options.attributeChangedCallback ||
			onChange ||
			consoleThis.bind(null, "onChange");
		Component.prototype.adoptedCallback =
			options.adoptedCallback ||
			consoleThis.bind(null, "adoptedCallback");
		Component.prototype.onCreated =
			options.onCreated || consoleThis.bind(null, "onCreated");
		...
		Object.keys(elemMethods).forEach((key) => {
			...
			var isDescriptor =
				...
				Object.keys(elemMethods[key]).some((key) =>
					descriptorKeys.includes(key),
				);
			if (isDescriptor) {
